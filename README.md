...
| `app.py` | Streamlit demo app — loads `best_model.pkl` and predicts churn risk for a customer profile entered through a form |
...
```bash
conda create -n telco-churn python=3.14
conda activate telco-churn
pip install -r requirements.txt
```

Or install into an environment you already have:
...
3. **Run `model_comparison.ipynb`** top to bottom. Loads `teleco_model_ready.csv`, compares models, produces `best_model.pkl`.
